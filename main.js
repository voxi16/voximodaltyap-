...
client.on("userUpdate", async (oldUser, newUser) => {
    if (oldUser.username !== newUser.username) {
      let tag = "そ"; 
      let sunucu = "786954981588205599"; 
      let kanal = "787383597384335360" 
      let rol = "786984419994238998"; // rol ID
	...
	newUser.send('Deuslra Tagını Bırakıp Aramızdan Ayrılmanın Nedenini Anlamış Değiliz Fakat Tekrar Aramıza Katılmak İstersen')
	newUser.send('Deuslra Tag: **そ**')
      }
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  let log = newState.guild.channels.cache.get(config.voicelog)
